...
mqtt_client.subscribe(f"bike/000001/resistance")
mqtt_client.get_client().on_message = message
mqtt_client.get_client().loop_start()

while 1:
    if(keyboard.is_pressed('space')):
        user_input = input("Enter resistence percentage: ")
        user_input = user_input.strip()
        if user_input.isdigit():
            user_input = int(user_input)
            dump = json.dumps({"value": user_input})
            mqtt_client.publish(f"bike/000001/resistance/control", user_input)
            print("Published resistence to MQTT Broker: ", str(user_input))
            time.sleep(1)
        else:
            print("invalid input")
